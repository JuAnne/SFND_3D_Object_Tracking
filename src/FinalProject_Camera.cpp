...
int main(int argc, const char *argv[])
{
            ...
            for (const auto& matcherType : matcherTypeList)
            {
                for (const auto& selectorType : selectorTypeList)
                {
                    ...
                    for (int i=0; i<10; i++)
                    {
                        ...
                        perfEval.ttcCamera[i] = 0.0f;
                        perfEval.ttcLidar[i] = 0.0f;
                    }
    ...
    for (auto& perfEval : perfEvalList)
    {
        ...
        for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
        {
            ...
            DataFrame frame;
            frame.cameraImg = img; //color image
            ...
            if (dataBuffer.size() > 1) // wait until at least two images have been processed
            {
                ...
                // store best BB matches in current data frame
                ...
                // loop over all BB match pairs in current data frame
                for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
                {
                    // find bounding boxes associates with current match
                    BoundingBox *prevBB, *currBB;
                    // key/value pair is accessed via map.begin()->first, map.begin()->second
                    ...
                    for (auto it3 = (dataBuffer.end() - 2)->boundingBoxes.begin(); it3 != (dataBuffer.end() - 2)->boundingBoxes.end(); ++it3)
                    {
                        if (it1->first == it3->boxID) // check wether current match partner corresponds to this BB
                        {
                            prevBB = &(*it3);
                        }
                    }
        ...
        } // eof main loop over all images
    } // eof loop over performance evaluation list

    // Create the performance evaluation report and save in .csv file
    reportFile << "detector,descriptor,#Img,#KptsPerFrame,#KptsMatched,TimeDetectorMs,TimeDescriptorMs,TimeMatchingMs,TTCLidar(s),TTCCamera(s)" << endl;
    for (auto& perfEval : perfEvalList)
    {
        for (int i=0; i<imgEndIndex; i++)
        {
            reportFile << perfEval.detectorType << ","
                        ...
                        << perfEval.timeMatching[i] << ","
                        << perfEval.ttcLidar[i] << ","
                        << perfEval.ttcCamera[i] << endl;
        }
        reportFile << endl;
    }
    reportFile.close();
